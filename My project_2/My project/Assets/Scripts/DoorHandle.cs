...
public class DoorHandle : XRBaseInteractable
{
    ...
    private bool doorOpened = false;  // New flag to lock door open state
    ...
    private void Start()
    {
        ...
        // Optionally disable visual lines until grabbed
    ...
    {
        // If door is open, ignore further interaction that would reset its position
        if (doorOpened)
            return;
    ...
    {
        // Make sure we only try to open it once.
        if (doorOpened)
            return;
    ...
    private IEnumerator OpenDoorCoroutine()
    {
        ...
        doorOpened = true; // Set flag to indicate door is open and lock state
